...
class AdComplaint extends My_Controller 
{
	...
	public function list() {
		if($this->role == 'admin'){
			...
			$params = $this->searchParam(['status','ticket_no','ga_no','complaint_type','classification']);
		}	
		...
		if($this->role == 'admin'){
			...
			$whereArr['h.emp_id'] = $this->userid;
	  		...
	  		if(isset($_GET['action']) && ($_GET['action'] == 'yes' || $_GET['action'] == 'no') ){
	  			$action_taken = $_GET['action'];
	  			if($action_taken == 'yes'){
	  				// $raw_where = " (h.type='remark' OR h.type='assign' AND solution != '') ";
	  				$raw_where = " (
	  					(h.type='remark') OR
	  					(h.type='assign' AND solution != '')
	  				) ";
	  			}else{
	  				/*$raw_where = " (h.type='assign' AND solution = '') ";
	  				$raw_where = " (
	  					(h.top_dept = 1 AND h.type='remark') OR
	  					(h.top_dept = 0 AND h.type='assign' AND solution = '')
	  				) ";*/
	  				$raw_where = " (
	  					(h.top_dept = 1 AND h.type NOT IN('assign','remark')) OR
	  					(h.top_dept = 0 AND h.type='assign' AND solution = '')
	  				) ";
	  			}
	  		}else{
	  			$raw_where = " (h.type='assign') ";
	  		}

	  		
  			## Total number of records without filtering
			$totalAssigned = $this->Complaint_model->get_complaints_for_emp($assignWhere,'c.id');
			$totalRecords = count($totalAssigned);

			## Total number of records with filtering
			$allRes = $this->Complaint_model->get_complaints_for_emp($whereArr,'c.id',FALSE,FALSE, $likeArr,$raw_where);
			$totalRecordwithFilter = count($allRes);

			##List
			$columns = 'c.id,c.ticket_no,c.ga_no,c.complaint_type,c.customer_id,c.status, c.created_at,c.classification,c.feedback';
			$list = $this->Complaint_model->get_complaints_for_emp($whereArr,$columns,$startrow, $rowperpage , $likeArr,$raw_where);
		...
		foreach ($list as $key => $value) {
			...
			if($this->role == 'employee'){
        		...
        		$raw_where = "(type='remark' OR solution != '')";

				$remark = $this->ComplaintHistory_model->get_complaint_history(['complaint_id' => $value['id'], 'emp_id' => $this->userid],'id',['raw_where' => $raw_where]);
	...
	public function getCompanyData() {
		...
		//get ga details
		$projects = $this->Project_model->get_projects(array('company_id' => $id, 'status' => 1), 'ga_no,equipment_id,model');
	...
	public function store(){
        ...
        if($projects){
        	...
        	sendResponse(0, 'Company and GA No mismatch');
        }
	...
	public function assign(){
		...
		if($notify_customer){
			...
			if($customer){
				...
				We writing this to inform you that your complaint (Ticket no. '.$complaint['ticket_no'].') have been assigned to '.$department_name.'. <br><br>
				...
				$mail_details[] = array(
					...
					'subject' => 'Your Complaint have been assigned to the respective department.'.ticketText($complaint['ticket_no']),
					'body' => $custBody,
					'cc' => $email_arr
				);
			}
	...
	private function enggVisitMail($data,$complaint){
		...
		if($customer){
			...
			$custSubject = 'Engineer Visit Has Been Scheduled.'.ticketText($complaint['ticket_no']);
			$custMsg 	= '
	...
	private function remarkMail($data,$complaint){
		...
		if($customer){
			...
			$custSubject = 'Ticket updates :'.ticketText($complaint['ticket_no']);
    ...
    {	
		...
		if($this->role == 'admin'){
			...
			$columns = '*';
			$list = $this->Complaint_model->get_complaints_for_emp($whereArr,$columns,FALSE,FALSE, $likeArr);
...
// die;
        ...
        foreach ($list as $key=>$row) {
            ...
            $sheet->setCellValueByColumnAndRow($col_count++, $excel_row, $row['customer']);
            $sheet->setCellValueByColumnAndRow($col_count++, $excel_row, $row['company']);
            $sheet->setCellValueByColumnAndRow($col_count++, $excel_row, $row['ga_no']);
            $sheet->setCellValueByColumnAndRow($col_count++, $excel_row, $row['equipment']); //Equipment
            $sheet->setCellValueByColumnAndRow($col_count++, $excel_row, $row['model']); //model
            $sheet->setCellValueByColumnAndRow($col_count++, $excel_row, $row['cust_equipment_no']);
            ...
            $sheet->setCellValueByColumnAndRow($col_count++, $excel_row, $row['order_no']);
            $sheet->setCellValueByColumnAndRow($col_count++, $excel_row, $row['description']);
