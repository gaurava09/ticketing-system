...
class CuComplaint extends My_Controller 
{
	...
	public function index() {
		...
		// dd($list);
		//get complaints
		$where_arr  = ['customer_id' => $this->userid];
	...
	public function searchComplaint() {
		...
		$where_arr  = ['customer_id' => $this->userid];
		if($ga_no){
			$where_arr['ga_no'] = $ga_no;
		}
		if($ticket_no){
			$where_arr['ticket_no'] = $ticket_no;
		}

		if($cust_equi_no){
			$where_arr['cust_equipment_no'] = $cust_equi_no;
		}

		if($complaint_type){
			$where_arr['complaint_type'] = $complaint_type;
		}
  ...
  //           ),
	...
	public function view($complaintId) {
		...
		$company = $this->Company_model->get_company(['id' => $this->company_id],'name');
